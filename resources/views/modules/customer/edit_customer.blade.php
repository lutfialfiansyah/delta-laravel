...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            <li>
                <a href="">{!! Breadcrumbs::render('edit.customer') !!}</a>
            </li>
        </ul>
                                ...
                                <div class="form-group">
                                    ...
                                    <select  required class="form-control select2" name="branch">
                                        ...
                                        @foreach($branch as $row)
                                            <option @if($data->branch_id==$row->id) selected @endif value="{{$row->id}}">{{$row->description}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" required name="customer_group">
                                        ...
                                        @foreach($customerGroups as $customerGroup)
                                            <option @if($data->customer_group_id==$customerGroup->id) selected @endif value="{{$customerGroup->id}}">{{$customerGroup->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" required name="salesman" tabindex="1">
                                        ...
                                        @foreach($salesman as $row)
                                            <option @if($data->salesman_id==$row->id) selected @endif value="{{$row->id}}">{{$row->code}} {{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" required name="payment_term"  tabindex="1">
                                        ...
                                        @foreach($paymentterms as $paymentterm)
                                            <option  @if($data->payment_term_id==$paymentterm->id) selected @endif value="{{$paymentterm->id}}">{{$paymentterm->name}} | {{$paymentterm->total_period}} | {{$paymentterm->percent_discount}}% | {{$paymentterm->discount_period}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        {{--<div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" required name="ar_limit" value="{{$data->ar_limit}}" class="form-control">
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('.btnsaveproduct').click(function () {
                var urls='{{route('customer.updateData',['id'=>$data->id])}}';
                if(form.valid()) {
                    $.ajax({
                        url: urls,
                        type: 'POST',
