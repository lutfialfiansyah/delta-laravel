...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            <li>
                <a href="">{!! Breadcrumbs::render('edit.purchaseReturn') !!}</a>
            </li>
        </ul>
...
<form action="" id="formPurchaser" method="" class="horizontal-form">
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="purchase_return_no" readonly value="{{$purchaseReturn->purchase_return_no}}" class="form-control">
                                </div>
                            </div>
                            <!--/span-->
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="date" required name="date" value="{{$purchaseReturn->date}}" class="form-control" placeholder="-">
                                ...
                                <div class="form-group">
                                    ...
                                    <select  required class="form-control select2" id="branch" style="width:100%" name="branch">
                                        ...
                                        @foreach($branch as $row)
                                            <option {{{($purchaseReturn->branch_id==$row->id)?'selected':''}}} value="{{$row->id}}">{{$row->description}}</option>
                                        @endforeach
                                    </select
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select name="warehouse_id" style="width: 100%"  class="form-control select2">
                                        ...
                                        @foreach($warehouse as $row)
                                            <option @if($purchaseReturn->warehouse_id==$row->id) selected @endif value="{{$row->id}}">{{$row->code}}-{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="supplier_id" style="width: 100%"  id="purchaseSpId" class="form-control select2">
                                        ...
                                        @foreach($supplier as $row)
                                            <option @if($purchaseReturn->vendor_id==$row->id) selected @endif  value="{{$row->id}}">{{$row->code}} -- {{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="shopping_method" style="width: 100%"  id="purchaseSmId" class="form-control select2">
                                        ...
                                        @foreach($shippingMethod as $row)
                                            <option @if($purchaseReturn->delivery_type_id==$row->id) selected @endif value="{{$row->id}}">{{$row->code}} -- {{$row->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="totalsum"  id="tsumtotal" value="{{$purchaseReturn->total}}" class="form-control">
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
                                    <label class="control-label">Remark</label>
                                    <textarea name='remark' class='form-control'>{{$purchaseReturn->remarks}}</textarea>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="detailpurchaseorder"  aria-labelledby="myModalLabel"  role="dialog" aria-hidden="true">
        <div class="modal-dialog cascading-modal" rule="document">
            ...
            <input type="hidden" name="idpo" value="{{$purchaseReturn->id}}">
        </div>
    </div>
    </form>
<script type="text/javascript">
    $(function () {
        ...
        $(document).on("click", '.editPurchseReturn',function(){
        ...
        $(".btnall").click(function () {
            ...
            if(id==0){
                $("#tabdetail").focus();
                var urls='{{route('purchaseReturn.addProduct')}}';
                $.ajax({
                    url: urls,
                    type: 'POST',
                ...
                var urls='{{url('api/purchaseReturn/updateData/')}}/'+id;
                hitung()
                $.ajax({
                    url: urls,
                    type: 'POST',
        ...
        $(".btnSaveProduct").click(function () {
            $.post("{{ route('purchaseReturn.updateProduct',$purchaseReturn->id)}}", $('#formPurchaser').serialize(),function(data){
                console.log(data)
                swal({
                        title: "Purchase",
